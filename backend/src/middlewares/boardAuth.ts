...
import { PrismaClient } from "@prisma/client"; // Role
const prisma = new PrismaClient();

// Hàm này sẽ kiểm tra xem user có phải là thành viên của board không
export const checkBoardMembership = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    if (!membership) {
      return res
        .status(403)
        .json({ message: "Bạn không phải là thành viên của board này" });
    }
...
export const checkBoardAdmin = (
  ...
  if ((req as any).role !== "ADMIN") {
    return res
      .status(403)
      .json({ message: "Bạn phải là ADMIN để thực hiện hành động này" });
  }
  next();
};

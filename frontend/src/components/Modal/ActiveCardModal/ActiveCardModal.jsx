...
function ActiveCardModal({ activeCard, isOpen, onClose, boardMembers }) {
  ...
  const handleAddComment = async () => {
    if (!comment) return;
    try {
      const newComment = await createNewCommentAPI(activeCard._id, {
        content: comment,
      });
  ...
  const handleToggleMember = async (userId) => {
    try {
      ...
      if (isAssigned) {
        ...
        activeCard.assignees = activeCard.assignees.filter(
          (a) => a.userId !== userId
        );
  ...
  const handleDeleteChecklist = async (checklistId) => {
    try {
      ...
      activeCard.checklists = activeCard.checklists.filter(
    ...
    <Modal open={isOpen} onClose={onClose}>
      <Box sx={style}>
        ...
        >
          ...
          <CloseIcon
            onClick={onClose}
            sx={{ cursor: "pointer", color: "#5e6c84" }}
          />
        </Box>
                  ...
                  <Box key={checklist.id} sx={{ mb: 3 }}>
                    ...
                    >
                      ...
                      <LinearProgress
                        variant="determinate"
                        value={progress}
                        sx={{ width: "100%", borderRadius: 4, height: 8 }}
                      />
                    </Box>
          ...
          <Grid item xs={3}>
            ...
            <Stack direction="column" spacing={1}>
                ...
                <Box sx={{ p: 2, width: 250 }}>
                  ...
                  <List dense>
                    {boardMembers?.map((member) => {
                      const isAssigned = activeCard.assignees?.some(
              ...
              <Button
                ...
                sx={{ justifyContent: "flex-start", boxShadow: "none" }}
              >
                Delete
              </Button>
            </Stack>
